...
def regenerate_stats(data):
    """Mirrors pipeline.py / fix_fallbacks.py so keep these aligned."""
    incidents = data['incidents']
    data['citywide']['mvas'] = sum(1 for i in incidents if i['type'] == 'MVA')

    per_district: dict = defaultdict(int)
    for i in incidents:
        if i['type'] == 'MVA':
    ...
    for k in sorted(by_month.keys()):
        ...
        new_trends.append({
            'month': k,
            'label': existing_labels.get(k, k),
            'totalCrimes': s['totalCrimes'],
            'shootings':   s['shootings'],
            'homicides':   s['homicides'],
            'mvas':        s['mvas'],
            'thefts':      s['thefts'],
            'stolenVehicles': s['stolenVehicles'],
        })
